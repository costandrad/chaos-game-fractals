...
function restricted_vertices(n; vertices = 1:4)
    v = Vector{Int}(undef, n)
    v[1] = rand(vertices)
    for i in 2:n
        v[i] = rand(setdiff(vertices, v[i-1]))
    end
    return v
end
...
polygon_vertices = [
    Point(
        polygon_radius * cos((k - 1) * central_angle - π/4),
        polygon_radius * sin((k - 1) * central_angle - π/4)
    ) for k in 1:num_sides
]

# Chaos-game orbit
orbit_points = [Point(0, 0)]
random_vertices = polygon_vertices[restricted_vertices(total_iterations - 1)]

for vertex in random_vertices
    push!(
        orbit_points,
        between(orbit_points[end], vertex, contraction_ratio)
    )
end
...
animation_name = replace(
    @sprintf(
        "%s_r%.3f_frames%d_fps%d_restricted_1",
        polygon_label,
        contraction_ratio,
        total_frames,
        frame_rate
...
function draw_background(scene, frame)
    ...
    setfont("Open Sans Extrabold", 64)
    settext(
        "Restricted",
        Point(0, -1.60 * polygon_radius),
        halign = "center", valign = "center"
    )
    settext(
        "Chaos Game",
        Point(0, -1.45 * polygon_radius),
        halign = "center", valign = "center"
    )

    setline(3)
    ngon(Point(0, 0), polygon_radius, num_sides, -π/4, action = :stroke)

    setfont("Open Sans Bold", 50)
    if num_sides == 3
        settext(
            @sprintf("%s", polygon_label),
            Point(0, -1.25 * polygon_radius),
            halign = "center", valign = "center"
        )
    else
        settext(
    ...
    settext(
        "The current vertex cannot be",
        Point(0, -1 * polygon_radius),
        halign = "center", valign = "center"
    )
    settext(
        "chosen in the next iteration",
        Point(0, -0.9 * polygon_radius),
        halign = "center", valign = "center"
    )
...
function draw_chaos_game(scene, frame)
    ...
    elseif frame <= 3frame_rate
        message = "3. Move P a fixed fraction r toward V."
        circle(orbit_points[1], 15, :fill)
        circle(random_vertices[1], 15, :fill)
        line(orbit_points[1], random_vertices[1], :stroke)
    ...
    elseif frame < 8frame_rate
        ...
        line(orbit_points[k], random_vertices[k], :stroke)
